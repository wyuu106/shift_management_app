...
function Calender({ period, shifts = [] }) {
  ...
  return (
    <section className="shift-calendar" aria-label="シフトカレンダー">
      <header className="shift-calendar__header">
        <div>
          <p className="shift-calendar__eyebrow">SHIFT CALENDAR</p>
      ...
      <div className="shift-calendar__legend" aria-label="表示の説明">
        <span><i className="legend-dot legend-dot--shift" />シフトあり</span>
        <span><i className="legend-dot legend-dot--closed" />休業日</span>
      </div>
      ...
      <div className="shift-calendar__months">
        {months.map((month) => {
          ...
          const visibleStart = isBefore(periodStart, monthStart) ? monthStart : periodStart;
          const visibleEnd = isAfter(periodEnd, monthEnd) ? monthEnd : periodEnd;
          const leadingCells = getDay(visibleStart);
          const dates = eachDayOfInterval({ start: visibleStart, end: visibleEnd });
            ...
            <article className="calendar-month" key={format(month, "yyyy-MM")}>
              ...
              <div className="calendar-grid">
                ...
                {Array.from({
                  ...
                  <span className="calendar-day calendar-day--empty" key={`tail-${index}`} />
                ))}
              </div>
            </article>
          );
