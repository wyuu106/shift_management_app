# シフト管理システム

アルバイト先の飲食店の業務を効率化するためのWebアプリケーションです。  
シフト管理、シフト希望提出などを一元化しています。

社員との話し合いも行い、実際に店舗に導入し、運用する予定です。

---

# 背景

私のアルバイト先の店舗では、シフト管理をLINEの日程調整機能と紙で行っており、アルバイトがLINEでシフト希望日を提出し、社員がそれをもとにシフトを作成して紙にまとめる運用でした。

しかし、この方法には以下の課題があると考えました。

- シフト希望に備考を書くことができない  
- LINEの日程調整やシフトの確認でトーク履歴を遡る必要がある  
- 紙にシフトをまとめて書く手間

これらの課題を解決するために本アプリの開発に取り組みました。

---

## 技術スタック

- Frontend: JavaScript React  
コンポーネント単位で画面を構築でき、再利用性や保守性の高いUIを実現しやすいため採用した。また、状態管理による動的な画面更新にも適していると考えた。

- Backend: Python FastAPI  
これまでの個人開発でも利用しており、API開発を効率的に進められることに加え、型ヒントや自動生成されるAPIドキュメントにより保守性の高い開発ができると考えた。

- Database: PostgreSQL  
同時アクセス性能が高く、複数の利用者が同時にアクセスするWebアプリケーションに適していると考え採用した。

- Authentication: JWT
- ORM: SQLAlchemy

---

## 機能一覧

### ユーザー機能
- ユーザー登録申請
- 登録申請の許可　/ 却下
- ログイン / ログアウト（JWT認証）
- 権限制御（管理者 / スタッフ）

### 業務関連
- シフト希望の提出
- シフト確認

↓（管理者機能）
- シフト登録
- シフト提出の期間を管理
- ユーザー管理

---

## 工夫した点

- JWT認証を用いたログイン機能を実装し、安全な認証・認可を実現
- 管理者とスタッフで利用できる機能を制限し、権限管理を実装
- データベースに保存する情報を必要最小限に抑え、保守性を考慮
- バックエンドとフロントエンド間でやり取りするデータを整理し、API設計を最適化
- モジュール間の依存を抑えた疎結合な設計を採用
- 機能追加や仕様変更に対応しやすい拡張性の高い設計を意識
- スタッフや社員へのヒアリングを行い、現場で使いやすいUI・操作性を追求

---

## 詳細

- ログイン画面

ユーザーネームとパスワードを入力してログインできます。

<p>
  <img src="images/login.png" width="40%">
</p>

- ユーザー登録申請

ユーザー名とパスワードを設定して、ユーザー登録申請を行うことができます。  
この時点では、管理者に登録申請が送信されるだけで、実際に登録はされません。

<p>
  <img src="images/register.png" width="40%">
</p>

- 管理者画面・スタッフ画面

管理者用の機能は「シフト期間の設定」「シフト登録」「ユーザー管理」「ユーザー登録申請の管理」です。
スタッフ用の機能は「シフト希望の提出」「シフト確認」です。

<table>
  <tr>
    <td valign="top">
      <img src="images/admin.png" width="80%">
    </td>
    <td valign="top">
      <img src="images/staff.png" width="80%">
    </td>
  </tr>
</table>

- シフト期間の設定（管理者機能）

シフト登録の期間を更新できます。シフト登録期間の開始日と終了日を選択した上で、その期間内の営業日を選択できます。
スタッフは、ここで設定された期間のシフト希望を提出できるようになります。

<img src="images/shift_period.png" width="60%">


- シフト登録（管理者機能）

スタッフから送られてきたシフト希望を参照しながら、シフトを登録することができます。ここで登録したシフトは、スタッフも閲覧できるようになります。

<br>

まず、「希望を取得」ボタンを押すと、スタッフからのシフト希望が反映されます。そして、各日付ごとにメンバーや備考を保存し、最後に「登録」ボタンを押すことで、期間内のシフトをまとめて登録します。

### シフト表
<p>
  <img src="images/shifts1.png" width="40%">
  <img src="images/shifts2.png" width="40%">
</p>

<br>

### 各日付のシフト保存
<img src="images/shift.png" width="40%">


- スタッフ管理（管理者機能）

<p>
  <img src="images/users.png" width="40%">
</p>

- 登録申請の許可・却下（管理者機能）

管理者がユーザー登録申請を許可した場合飲み、そのユーザーが登録されます。

<p>
  <img src="images/user_request.png" width="40%">
</p>

- シフト希望提出（スタッフ用機能）

期間内の日付から自分が出勤可能な日を選んで、シフト希望を提出することができます。備考を追加することも可能です。

<img src="images/shift_request.png" width="40%">

- シフト確認（スタッフ用機能）

確定したシフトを確認すうことができます。

<img src="images/staff_shifts.png" width="40%">
